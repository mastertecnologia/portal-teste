...
**Pastas/arquivos que devem estar no Linux:**

| Copiar | Observação |
|--------|------------|
| `bin/` | Comandos CakePHP |
| `config/` | Inclui exemplos; no Linux use `app_local.php` (script cria a partir do example) |
| `src/` | Controllers, Models, Templates – **aqui está a ligação com o ERP** (veja `LIGACAO_ERP_WINDOWS.md`) |
| `vendor/` | Dependências + PGMPackages (UserConstants.php com C_ChaveAcesso, C_Filial) |
| `webroot/` | CSS, JS, plugins, index.php |
| `scripts/` | preparar_linux.sh, test_ambiente_linux.php |
| `.htaccess` | Raiz e dentro de webroot |
| `.env.example` | Opcional; para usar variáveis de ambiente |
...
1. Cria os diretórios `tmp`, `tmp/cache`, `logs`, `webroot/arquivos/tickets` se não existirem  
2. Copia `config/app_local_linux.example` para `config/app_local.php` (só se ainda não existir)  
3. Ajusta permissões (775 em tmp, logs, webroot/arquivos)  
...
- **Diretório não gravável:** use `./scripts/preparar_linux.sh www-data` ou `sudo chown -R www-data:www-data /var/www/portal/tmp /var/www/portal/logs /var/www/portal/webroot/arquivos`.  
...
- **Apache:** DocumentRoot = `/var/www/portal/webroot`. Ver exemplos em `MIGRACAO_LINUX.md`.  
- **Nginx:** `root /var/www/portal/webroot;` e `try_files $uri $uri/ /index.php?$args;`. Ver `MIGRACAO_LINUX.md`.
...
| `config/app_local.php` | Config local (não versionado); edite host, DB, salt |
| `scripts/preparar_linux.sh` | Cria dirs, config e roda teste |
| `scripts/test_ambiente_linux.php` | Testa paths e conexão PostgreSQL |
| `MIGRACAO_LINUX.md` | Detalhes de estrutura, Apache/Nginx, permissões |
