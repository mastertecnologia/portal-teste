	...
	public function add() {
		...
		if ($this->request->is('post')) {
            ...
            if ($this->Ordensservico->save($ordem)) {
				$session = $this->request->getSession();
				$idcarrinhoSession = $session->read('PGM_Ordem_Idcarrinhoadd');
				$carrinho = $this->Ordemservicositens->newEntity();
				$idempresaCarrinho = $idcarrinhoSession[0] ?? null;
				if ($idempresa == $idempresaCarrinho) {
					$carrinho->iditens = $idcarrinhoSession;
				} else {
					$carrinho->iditens = $idempresa . substr($idcarrinhoSession, 1);
					$itemOrdens = $this->Itensordem->find('all')->where(['idordempk' => $idcarrinhoSession])->toArray();
				...
				$session->delete('PGM_Ordem_Idcarrinhoadd');
...
/* 		$clientes = $this->Clientes->find('all', ['keyField' => 'id', 'valueField' => 'razaosocial'])->where(['idempresa' => $idempresa, 'inativo' => 0])->order(['razaosocial'])->toArray();
	...
	public function carrinho($idordem = null){
		...
		if ($idordem === null) {
			$idordem = $this->request->getSession()->read('PGM_Ordem_Idcarrinhoadd');
	...
	public function carrinhoadd($idordem = null, $codproduto = null){
        ...
        $ordem->idempresa =  $this->Auth->user('idempresa');
        ...
        $ordem->valorunitario = formatNumber($data['valorunitario']);
        $ordem->valordesconto = formatNumber($data['valordesconto']);
        $ordem->valortotal = formatNumber($data['valortotal']);
	...
	public function listAPI() {
        ...
        if ($this->request->is('get')) {
			...
			if($token == $this->Empresas->get($empresa)->token) {
				if(!empty($id)){
					$ordem = $this->Ordensservico->find('all')->where(['Ordensservico.idempresa' => $empresa, 'Ordensservico.id' => $id, 'situacao' => $situacao])
						->contain([
							...
							'Users' => ['fields' => ['Users.id', 'Users.cpf']],
							'Ordemservicositens',
							'Ordemparcelas' => function ($q) use ($empresa) {
								return $q->where(['Ordemparcelas.idempresa' => $empresa]);
							}
						])
					->toArray();
				...
				// Carrega itens em lote (evita N+1)
				$iditensList = [];
				foreach ($ordem as $reg) {
					$firstCarrinho = !empty($reg->ordemservicositens) ? $reg->ordemservicositens[0] : null;
					if ($firstCarrinho && !empty($firstCarrinho->iditens)) {
						$iditensList[] = $firstCarrinho->iditens;
					}
				}
				$itensByPk = [];
				if (!empty($iditensList)) {
					$iditensList = array_unique($iditensList);
					$itensRecords = $this->Itensordem->find()->where(['idordempk IN' => $iditensList])->order(['idordempk', 'id'])->all();
					foreach ($itensRecords as $i) {
						$itensByPk[$i->idordempk][] = $i;
					}
				}
				foreach($ordem as $reg){
					$firstCarrinho = !empty($reg->ordemservicositens) ? $reg->ordemservicositens[0] : null;
					$itens = [];
					if ($firstCarrinho && !empty($firstCarrinho->iditens) && isset($itensByPk[$firstCarrinho->iditens])) {
						$itens = $itensByPk[$firstCarrinho->iditens];
					}
					$reg->itens = $this->itensArr($itens);
					$reg->pagamento = $this->parcelasArr($reg->ordemparcelas ?? []);
	...
	public function ordensArr($ordem){
		...
		$ordem->cpftecnico = !empty($ordem->user) ? $ordem->user->cpf : $this->Users->get($ordem->iduser)->cpf;
		unset($ordem->id);
		unset($ordem->idcliente);
		unset($ordem->iduser);
		unset($ordem->valortotal);
		unset($ordem->situacao);
		unset($ordem->relato);
    ...
    public function refreshAPI() {
        ...
        if ($this->request->is('put')) {
			...
			if($token == $this->Empresas->get($empresa)->token){
				...
				if ($ordem == null) return $this->jsonResponse(['mensagem' => 'Parâmetros da requisição inválidos.'], 400);
                ...
                if($this->Ordensservico->save($ordem)){
					$this->Ordensservico->criarMov($ordem->id, $sitantiga, $ordem->situacao, $empresa, $this->Auth->user('idempresa'), $this->Auth->user('id'));
					return $this->jsonResponse(['mensagem' => 'Situação da Ordem de Serviço alterada com sucesso'], 201);
				}
                else return $this->jsonResponse(['mensagem' => 'Ocorreu um erro ao atualizar a ordem!'], 400);
	...
	public function imprimirordens(){
		...
		$ordens = $this->Ordensservico->find('all')
			->contain([
				'Users',
				'Clientes' => ['Cidades' => 'Estados'],
				'Ordemservicositens'
			])
			->where(['Ordensservico.id IN' => explode(',', $data['idsimprimir']), 'Ordensservico.idempresa' => $idempresa])
			->toArray();

		$iditensList = [];
		foreach ($ordens as $ordem) {
			$firstCarrinho = !empty($ordem->ordemservicositens) ? $ordem->ordemservicositens[0] : null;
			if ($firstCarrinho && !empty($firstCarrinho->iditens)) {
				$iditensList[] = $firstCarrinho->iditens;
			}
		}
		...
		$carrinhos = [];
		foreach($ordens as $ordem) {
			if (!empty($ordem->cliente->cidade)) {
				$cidades[$ordem->id] = $ordem->cliente->cidade;
				$estados[$ordem->id] = !empty($ordem->cliente->cidade->estado) ? $ordem->cliente->cidade->estado : null;
			}
			$firstCarrinho = !empty($ordem->ordemservicositens) ? $ordem->ordemservicositens[0] : null;
			$carrinhos[$ordem->id] = [];
			if ($firstCarrinho && !empty($firstCarrinho->iditens) && isset($itensByPk[$firstCarrinho->iditens])) {
				$carrinhos[$ordem->id] = $itensByPk[$firstCarrinho->iditens];
			}
		}
	...
	public function ticketordem($idticket) {
		...
		if ($this->request->is('post')) {
            ...
            if ($this->Ordensservico->save($ordem)) {
				// Itens
				$carrinho = $this->Ordemservicositens->newEntity();
				$carrinho->iditens = $session->read('PGM_Ordem_Idcarrinhoadd');
				$carrinho->idordem = $ordem->id;
				$carrinho->idempresa = $idempresa;
				$this->Ordemservicositens->save($carrinho);
				$session->delete('PGM_Ordem_Idcarrinhoadd');
	...
	public function acaoindex() {
		...
		if (empty($ids)) {
			$this->Flash->error(__('Nenhuma ordem selecionada.'));
		...
		foreach ($ids as $id) {
			...
			if(!empty($ordem)) {
				...
				if(in_array($data['situacao'], [C_OrdensSituacaoLiberadaParaFaturamento, C_OrdensSituacaoCancelada])) {
					$ticket = $ordem->ticket ?? $this->Tickets->findById($ordem->idticket)->where(['idempresa' => $idempresa])->first();
					if(!empty($ticket)) {
						$sitantiga = $ticket->situacao;
