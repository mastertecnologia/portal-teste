  ...
  if (data.tab === 'pecas') {
    ...
    return (
      <div>
        ...
        {pecasModalOpen ? (
          <div
            className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/55 px-2 py-2"
            role="dialog"
            aria-modal="true"
            onClick={() => setPecasModalOpen(false)}
          >
            <div
              className="flex h-[96vh] w-[98vw] max-w-[98vw] flex-col rounded-xl border border-[var(--pgm-border)] bg-[var(--pgm-bg-surface)] p-3 shadow-xl"
              onClick={(ev) => ev.stopPropagation()}
            >
              ...
              <div className="mb-2 grid gap-2 sm:grid-cols-[1fr_180px_180px]">
                <input
                  type="search"
                  value={pecasCatalogQ}
              ...
              <div className="min-h-0 flex-1 overflow-auto rounded-md border border-[var(--pgm-border-subtle)]">
                <table className="w-full table-fixed text-left text-xs">
                  <thead>
                    <tr className="border-b border-[var(--pgm-border)] text-xs text-[var(--pgm-text-muted)]">
                      <th className="sticky top-0 z-10 w-[7%] bg-[var(--pgm-bg)] px-2 py-2">Código</th>
                      <th className="sticky top-0 z-10 w-[37%] bg-[var(--pgm-bg)] px-2 py-2">Descrição</th>
                      <th className="sticky top-0 z-10 w-[8%] bg-[var(--pgm-bg)] px-2 py-2">Tipo</th>
                      <th className="sticky top-0 z-10 w-[9%] bg-[var(--pgm-bg)] px-2 py-2 text-right">Qtde</th>
                      <th className="sticky top-0 z-10 w-[10%] bg-[var(--pgm-bg)] px-2 py-2 text-right">Custo</th>
                      <th className="sticky top-0 z-10 w-[10%] bg-[var(--pgm-bg)] px-2 py-2 text-right">Venda</th>
                      <th className="sticky top-0 z-10 w-[7%] bg-[var(--pgm-bg)] px-2 py-2 text-right">Qtd</th>
                      <th className="sticky top-0 z-10 w-[8%] bg-[var(--pgm-bg)] px-2 py-2 text-right">Total</th>
                      <th className="sticky top-0 z-10 w-[8%] bg-[var(--pgm-bg)] px-2 py-2 text-right">Ação</th>
                    </tr>
                  </thead>
                  <tbody>
                    {pecasCatalogItems.map((it, idx) => {
                      ...
                      return (
                        ...
                        >
                          <td className="px-2 py-2 whitespace-nowrap">{it.codigo || '—'}</td>
                          <td className="px-2 py-2">
                            <div className="line-clamp-2 leading-4 text-[var(--pgm-text)]">{it.descricao || '—'}</div>
                            {!it.tem_cadastro_portal ? (
                              <div className="mt-0.5 text-[0.65rem] text-amber-300">Sem cadastro local no portal</div>
                            ) : null}
                          </td>
                          <td className="px-2 py-2 whitespace-nowrap">{it.tipo === 'servico' ? 'Serviço' : 'Produto'}</td>
                          <td className="px-2 py-2 whitespace-nowrap text-right">{it.tipo === 'produto' ? (it.estoque ?? '0') : '—'}</td>
                          <td className="px-2 py-2 whitespace-nowrap text-right">{it.preco_custo != null ? BR.format(it.preco_custo) : '—'}</td>
                          <td className="px-2 py-2 whitespace-nowrap text-right">{BR.format(Number(it.preco_venda || valor || 0))}</td>
                          <td className="px-2 py-2 text-right">
                            <input
                              ...
                              className="w-full rounded-md border border-[var(--pgm-border)] bg-[var(--pgm-bg-raised)] px-1.5 py-1 text-xs text-[var(--pgm-text)]"
                            />
                          </td>
                          <td className="px-2 py-2 whitespace-nowrap text-right">{BR.format(totalLinha)}</td>
                          <td className="px-2 py-2 text-right">
                            <button
                              type="button"
