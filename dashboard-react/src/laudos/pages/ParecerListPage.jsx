...
/**
 * Listagem de Pareceres Técnicos — alinhada a `laudos-package/frontend/pages/ParecerListPage.jsx`.
 * Integração CakePHP: `boot` opcional com `onOpen(id)`, `onNew(data)`; padrão usa redirecionamento.
 */
export default function ParecerListPage({ boot }) {
  const onOpen = boot?.onOpen ?? ((pid) => { window.location.href = `/laudos/pareceres/${pid}`; });
  const onNew = boot?.onNew ?? ((data) => { if (data?.id) window.location.href = `/laudos/pareceres/${data.id}`; });
