...
- **ERP (role 0):** `AdvancedContracts`, `AdvancedInvoices`, `AdvancedAttendance`, `AdvancedReports`. Rotas explícitas em `config/routes.php` sob `/modulo-avancado/…` (listagem, detalhe, export CSV de faturas/indicadores, POST `marcar-paga` para fatura).
