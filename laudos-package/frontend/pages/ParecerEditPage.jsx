...
import { PareceresAPI, ProdutosAPI } from '../services/api';
...
/**
 * Página de edição de um parecer.
 *
 * Props:
 *   id: id do parecer (vindo da rota)
 *   onBack: () => void  — volta para listagem
 *
 * Adapte para seu sistema de rotas:
 *   const { id } = useParams();
 *   const navigate = useNavigate();
 *   <ParecerEditPage id={id} onBack={() => navigate('/laudos/pareceres')} />
 */
export default function ParecerEditPage({ id, onBack }) {
  ...
  // Auto-save de campos do próprio parecer (não filhos)
  const saveData = useMemo(() => {
    if (!parecer) return null;
    // Só campos do próprio parecer (sem produtos/anexos/historico)
    const {
  ...
  const { saveStatus, savedAt } = useDebounceSave({
    ...
    onSave: async (data) => {
      ...
      // atualiza totais (que podem ter mudado)
      if (resp.data) {
  ...
  // Aplica mudanças parciais ao parecer (otimista)
  ...
  const handleAddProduto = async () => {
    try {
      const resp = await ProdutosAPI.create({
        parecer_id: id,
  ...
  const handleProdutoChange = (idx, produtoUpdated) => {
    ...
    // Recalcula totais localmente (preview)
