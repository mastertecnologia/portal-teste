...
class ProdutosController extends AppController {
	...
	/**
	 * Tela de precificação com base em custo/preço ERP.
	 */
	public function precificacao() {
		$this->set('title', 'Gestão de Preços');
		$idempresa = (int) $this->Auth->user('idempresa');

		$regs = $this->Produtos->find()
			->where([
				'idempresa' => $idempresa,
				'tipo IN' => [1, 2, 3],
			])
		...
		// Mapa ERP por código (somente referência para cálculo).
		$erpMap = [];
		try {
			$empresa = $this->Empresas->get($idempresa);
			if (!empty($empresa->urlerp)) {
				$soapprodutos = $empresa->urlerp . 'WsProdutos.wso?wsdl';
				$this->runSoapBuffered(function () use ($soapprodutos, &$erpMap) {
					$soap = new CakeSoap(['wsdl' => $soapprodutos]);
					if ($soap === null) {
						throw new \Exception('Erro ao instanciar SOAP.');
					}
					$response = $soap->sendRequest('GetEstoqueProdutos', [
						'Data' => [
							'iFilial' => C_Filial,
					...
					foreach ($lista as $item) {
						...
						$erpMap[$cod] = [
							'custo' => isset($item->nPrecoCusto) ? (float)$item->nPrecoCusto : null,
							'venda' => isset($item->nPrecoVenda) ? (float)$item->nPrecoVenda : null,
							'qtde' => isset($item->nQtdeAtual) ? (float)$item->nQtdeAtual : 0.0,
						];
					}
				});
			}
		} catch (\Throwable $e) {
			$this->log('Produtos::precificacao ERP indisponível: ' . $e->getMessage(), 'warning');
		}

		$itens = [];
		foreach ($regs as $r) {
			$codigo = trim((string)$r->codigo);
			$erp = isset($erpMap[$codigo]) ? $erpMap[$codigo] : null;
			$custo = ($erp && $erp['custo'] !== null) ? (float)$erp['custo'] : null;
			$vendaAtual = (float)$r->vlunitario;
			$qtde = ($erp && $erp['qtde'] !== null) ? (float)$erp['qtde'] : 0.0;
			$markupAtual = ($custo !== null && $custo > 0 && $vendaAtual > 0)
				? (($vendaAtual / $custo) - 1) * 100
				: null;
			$margemAtual = ($custo !== null && $custo > 0 && $vendaAtual > 0)
				? (1 - ($custo / $vendaAtual)) * 100
				: null;
			...
			$itens[] = [
				...
				'qtde' => $qtde,
				'temCusto' => ($custo !== null && $custo > 0),
				'markupAtual' => $markupAtual,
				'margemAtual' => $margemAtual,
			];
		}

		$this->set('produtosJson', json_encode(
			$itens,
	...
	/**
	 * Salva preços em lote para produtos/serviços/contratos da empresa.
	 */
	public function salvarPrecos() {
		$this->request->allowMethod(['post']);
		$this->autoRender = false;
		...
		$idempresa = (int) $this->Auth->user('idempresa');
		$data = $this->request->getData();
		$precos = isset($data['precos']) ? $data['precos'] : null;
		if ($precos === null || !is_array($precos)) {
			$raw = $this->request->input('json_decode', true);
			$precos = is_array($raw) && isset($raw['precos']) && is_array($raw['precos']) ? $raw['precos'] : [];
		}
		if (empty($precos)) {
			return $this->jsonResponse(['salvos' => 0, 'erros' => [['mensagem' => 'Nenhum preço informado']]], 400);
		}
		...
		foreach ($precos as $item) {
			...
			if ($id <= 0 || $vl === null || $vl < 0) {
				$erros[] = ['id' => $id, 'mensagem' => 'Parâmetros inválidos'];
				continue;
			}

			$produto = $this->Produtos->find()
				->where(['id' => $id, 'idempresa' => $idempresa])
				->first();
			if (!$produto) {
				$erros[] = ['id' => $id, 'mensagem' => 'Registro não encontrado para a empresa'];
				continue;
			}
			...
			if ($this->Produtos->save($produto)) {
				...
				$erros[] = ['id' => $id, 'mensagem' => 'Falha ao salvar'];
			}
		}
