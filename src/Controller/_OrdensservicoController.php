...
class OrdensservicoController extends AppController {
    ...
    public function beforeFilter(Event $event) {
		...
		$this->Auth->allow(['refreshApi', 'listApi', 'refreshAPI', 'listAPI']);	
	}
...
/* 		$clientes = $this->Clientes->find('all', ['keyField' => 'id', 'valueField' => 'razaosocial'])->where(['idempresa' => $idempresa, 'inativo' => 0])->order(['razaosocial'])->toArray();
	...
	public function listAPI() {
        ...
        $hSituacao = $this->request->getHeaderLine('situacao');
        $hId = $this->request->getHeaderLine('id');
        $this->log('[API-ORDENS listAPI] request method=' . $reqMethod . ' path=' . $reqPath . ' headers(empresa=' . ($hEmpresa ?: 'vazio') . ' token=' . (strlen($hToken ?? '') ? '***' : 'vazio') . ' situacao=' . ($hSituacao !== '' ? $hSituacao : 'vazio') . ' id=' . ($hId ?: 'vazio') . ')', 'info');

        if (!$this->request->is('get') && !$this->request->is('post')) {
            $this->log('[API-ORDENS listAPI] resposta 405 metodo nao permitido', 'info');
            return $this->jsonResponse([
                'mensagem' => 'Método não permitido. Use GET ou POST com empresa, token e situacao (ex.: 4 = Liberada para faturamento).',
                'metodo_esperado' => 'GET ou POST',
            ], 405);
        }
        try {
            $data = $this->request->getData();
            ...
            if (empty($empresa) && isset($data['empresa'])) $empresa = $data['empresa'];
            if (empty($token) && isset($data['token'])) $token = $data['token'];
            if (($situacao === '' || $situacao === null) && array_key_exists('situacao', $data)) $situacao = $data['situacao'];
            if (empty($id) && isset($data['id'])) $id = $data['id'];
            $empresa = is_string($empresa) ? trim((string)$empresa) : (string)$empresa;
            $token = is_string($token) ? trim((string)$token) : (string)$token;
            $situacao = is_string($situacao) ? trim((string)$situacao) : (string)$situacao;
            $id = is_string($id) ? trim((string)$id) : (string)$id;

            $situacaoInt = ($situacao !== '' && $situacao !== null && is_numeric($situacao)) ? (int) $situacao : null;

            if (empty($token) || empty($empresa) || $situacaoInt === null) {
                $this->log('[API-ORDENS listAPI] resposta 400 parametros invalidos (empresa=' . ($empresa ?: 'vazio') . ' situacao=' . ($situacaoInt !== null ? $situacaoInt : 'vazio') . ')', 'info');
                return $this->jsonResponse([
                    'mensagem' => 'Parâmetros da requisição inválidos. Envie empresa, token e situacao (ex.: 4 para ordens liberadas para faturamento).',
                    'exemplo_get' => '/ordensservico/list-api?empresa=1&token=SEU_TOKEN&situacao=4',
                ], 400);
            }

            $empresaEntity = $this->Empresas->findById($empresa)->first();
            if (empty($empresaEntity)) {
                $this->log('[API-ORDENS listAPI] resposta 400 empresa nao encontrada id=' . $empresa, 'info');
                return $this->jsonResponse(['mensagem' => 'Empresa não encontrada.'], 400);
            }
            $tokenEsperado = $this->Empresas->get($empresa)->token;
            if ($token !== $tokenEsperado) {
                $this->log('[API-ORDENS listAPI] resposta 401 autenticacao invalida empresa=' . $empresa, 'info');
                return $this->jsonResponse(['mensagem' => 'Autenticação Inválida'], 401);
            }

			if (true) {
				if(!empty($id)){
					$ordem = $this->Ordensservico->find('all')->where(['Ordensservico.idempresa' => $empresa, 'Ordensservico.id' => $id, 'Ordensservico.situacao' => $situacaoInt])
						->contain([
					...
					$ordem = $this->Ordensservico->find('all')->where(['Ordensservico.idempresa' => $empresa, 'Ordensservico.situacao' => $situacaoInt])
						->contain([
							'Clientes' => ['fields' => ['Clientes.cnpj', 'Clientes.cpf', 'Clientes.razaosocial', 'Clientes.nome', 
							'Clientes.inscricaoestadual', 'Clientes.endereco', 'Clientes.nroendereco', 'Clientes.complemento', 'Clientes.bairro', 'Clientes.idcidade',
							'Clientes.cep', 'Clientes.fone', 'Clientes.email', 'Clientes.contrato', 'Clientes.tipo']]
				...
				foreach($ordem as $reg){
					try {
						...
						if ($idcarrinho && !empty($idcarrinho->iditens)) {
						...
						$this->log('Ordensservico::listAPI ordem ' . $reg->id . ': ' . $e->getMessage(), 'error');
						$reg->itens = $reg->itens ?? [];
						$reg->pagamento = $reg->pagamento ?? [];
						$reg = $this->ordensArr($reg);
					}
				}
				$this->log('[API-ORDENS listAPI] resposta 200 ok total ordens=' . count($ordem) . ' empresa=' . $empresa . ' situacao=' . $situacaoInt, 'info');
				return $this->jsonResponse($ordem, 200);
			}
        } catch (\Throwable $e) {
            $this->log('[API-ORDENS listAPI] resposta 500 excecao: ' . $e->getMessage() . ' em ' . $e->getFile() . ':' . $e->getLine(), 'error');
            $this->log('Ordensservico::listAPI erro: ' . $e->getMessage() . ' em ' . $e->getFile() . ':' . $e->getLine(), 'error');
            return $this->jsonResponse([
                'mensagem' => 'Erro ao listar ordens. Verifique os parâmetros (empresa, token, situacao).',
                'erro' => $e->getMessage(),
            ], 500);
        }
	}
