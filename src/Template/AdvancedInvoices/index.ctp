...
$advInvCount = isset($params['count']) ? (int)$params['count'] : (is_countable($invoices) ? count($invoices) : 0);
?>
	...
	<div class="pgm-adv-panel card">
		<div class="card-body">
			...
			<p class="small mb-3">
				...
				<span class="text-muted ml-2">Faturamento clássico: <?= $this->Html->link('Faturamento', '/faturamento') ?> · <?= $this->Html->link('Locação', '/locacao') ?></span>
			</p>
				...
				<table class="table table-sm table-striped mb-0">
					...
					<tbody>
						...
						<tr>
							...
							<td><?= !empty($inv->cliente) ? h($inv->cliente->razaosocial ?: $inv->cliente->nome) : '—' ?></td>
							<td><?= h($inv->total) ?></td>
							<td><?= h($inv->status) ?></td>
							<td><?= $this->Html->link('Ver', ['action' => 'view', $inv->id], ['class' => 'btn btn-sm btn-outline-primary']) ?></td>
						</tr>
						<?php endforeach; ?>
						<?php if ($advInvCount === 0): ?>
						<tr>
							<td colspan="6" class="text-muted">Nenhuma fatura no módulo avançado (<code>invoices</code>). Use o shell de geração mensal ou consulte o faturamento legado nos links acima.</td>
						</tr>
						<?php endif; ?>
					</tbody>
				</table>
