...
$dataEmissao = $doc->data_emissao ? $doc->data_emissao->format('d/m/Y') : date('d/m/Y');
$dataVenc = $doc->data_vencimento ? $doc->data_vencimento->format('d/m/Y') : '';

$linhasItens = [];
if (!empty($doc->faturamento_itens)) {
	foreach ($doc->faturamento_itens as $fi) {
		$linhasItens[] = $fi;
	}
}
...
$valorTotalHidden = isset($doc->valor_total) ? number_format((float)$doc->valor_total, 2, '.', '') : '0';
?>
<style>
...
<div class="fatadd-root">
	...
	<div class="fatadd-card">
		...
		<table class="fatadd-items-table" id="tblItens">
			...
			<tbody id="itensBody">
				<?php foreach ($linhasItens as $idx => $item) :
					$desc = $item ? h($item->descricao) : '';
					$qtd = $item ? (float)$item->quantidade : 1;
					$vuni = $item ? (float)$item->valor_unitario : 0;
					$vtot = $item ? (float)$item->valor_total : ($qtd * $vuni);
					?>
				<tr class="item-row">
...
<script>
var itemIdx = <?= max(count($linhasItens), 1) ?>;
