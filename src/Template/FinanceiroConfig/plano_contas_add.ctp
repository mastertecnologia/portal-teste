...
$this->Breadcrumbs->add('Plano de Contas', ['action' => 'planoContas']);
$this->Breadcrumbs->add('Nova conta');
$tipoOpts = ['receita' => 'Receita', 'despesa' => 'Despesa'];
$naturezaOpts = ['analitica' => 'Analítica (lançamentos)', 'sintetica' => 'Sintética (agrupadora)'];
?>
<style>
...
<div class="fin-root">
    <div class="fin-topbar">
        <h1 class="fin-h1"><i class="fas fa-plus fin-h1-ico"></i>Nova conta</h1>
        <div><?= $this->Html->link('Voltar', ['action' => 'planoContas'], ['class' => 'btn btn-default btn-sm']) ?></div>
    </div>
    ...
    <div class="fin-card">
        ...
        <div class="fin-row">
            <div class="fin-field">
                <label>Código</label>
                <?= $this->Form->control('codigo', ['label' => false, 'class' => 'form-control', 'placeholder' => 'Ex: 1.1.01']) ?>
            </div>
            <div class="fin-field">
        ...
        <div class="fin-row">
            <div class="fin-field">
                <label>Tipo</label>
                <?= $this->Form->select('tipo', $tipoOpts, ['class' => 'form-control', 'empty' => '— Selecione —']) ?>
            </div>
            <div class="fin-field">
                <label>Natureza</label>
                <?= $this->Form->select('natureza', $naturezaOpts, ['class' => 'form-control']) ?>
            </div>
            <div class="fin-field">
                <label>Conta pai (opcional)</label>
                <?= $this->Form->select('parent_id', $pais, ['class' => 'form-control', 'empty' => '— Nenhuma (raiz) —']) ?>
            </div>
        </div>
