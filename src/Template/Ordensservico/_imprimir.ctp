...
<style>
    ...
    #printable.pdf-mode .text-right {
        text-align: right;
    }

    #printable.pdf-mode .text-left {
        text-align: left;
    }

    #printable.pdf-mode .text-center {
        text-align: center;
    }
    ...
    @media print {
        ...
        .topbar, .page-titles, .navbar, .breadcrumb,
        .btn, #btn-imprimir, #btn-salvar-pdf {
...
function gerarPDF() {
    ...
    // Remover classes de estilo para impressão
    $(clone).find('.titulo').removeClass('bg-dark').removeClass('text-white');
    
    // Configurações do PDF
    const options = {
        margin: [10, 10, 10, 10],
        filename: `ordem_servico_<?= $ordem->id ?>.pdf`,
...
$('#btn-imprimir').click(function(e) {
    ...
    if ($("body").hasClass("mini-sidebar")) {
        ...
        $("#printable").addClass('printMini');
    }
    
    var $print = $('#printable').clone().addClass('print');
    window.print();
