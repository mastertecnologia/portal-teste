<?php
$this->assign('title', $title ?? __('Franquia'));
$rows = $rows ?? [];
$mes = $mes ?? date('Y-m');
?>
<div class="col-12 pgm-adv-page">
	<div class="pgm-adv-panel card mb-3">
		<div class="card-body">
			...
			<p class="text-muted small"><?= __('Horas consumidas no mês de referência.') ?></p>
			<form method="get" action="" class="form-inline mb-3">
				<label class="mr-2 small"><?= __('Mês (AAAA-MM)') ?></label>
				...
				<table class="table table-sm table-striped mb-0">
					<thead>
						<tr>
							<th><?= __('Contrato') ?></th>
							<th><?= __('Código') ?></th>
							<th><?= __('Horas incluídas') ?></th>
							<th><?= __('Consumidas (mês)') ?></th>
							<th><?= __('Saldo') ?></th>
						</tr>
					</thead>
					<tbody>
						...
						<?php endforeach; ?>
						<?php if (count($rows) === 0): ?>
						<tr><td colspan="5" class="text-muted"><?= __('Sem contratos ou sem dados para este mês.') ?></td></tr>
						<?php endif; ?>
					</tbody>
				</table>
			</div>
			<p class="mb-0 mt-2"><?= $this->Html->link(__('← Contratos'), ['action' => 'index'], ['class' => 'btn btn-sm btn-secondary']) ?></p>
		</div>
	</div>
</div>
