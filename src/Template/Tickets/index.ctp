						...
						<table class="table table-hover table-row-clickable" id="table-todos">
							...
							<tbody>
								<?php foreach ($tickets as $reg): ?>
									<tr rel="popover" data-trigger="hover" data-content='<div class="popover-big"><h4><?= AssuntoTicket($reg->assunto) ?> </h4><br><?= h($reg->solicitacao) ?></div>' data-original-title="Ticket <?= $reg->id.' ' ?><small class='pgm-popover-title-date'><i>(<?= date_format($reg->created, 'd/m/Y') ?>)</i></small>" data-html="true" data-placement="top">
										...
										<td class="td-actions">
											<?php 
												if($reg->situacao != C_TicketSituacaoResolvido && $reg->situacao != C_TicketSituacaoFechado) {
													...
													if($reg->situacao != C_TicketSituacaoEmandamento) echo $this->Html->link('<i class="fas fa-reply"></i>', ["action" => "alterarsituacao", $reg->id, '1'], ['rel' => 'tooltip', 'title' => 'Em execução', 'class' => 'btn btn-pgm btn-pgm-situacao btn-info btn-simple btn-xs', 'id' => $reg->id, 'escape' => false]);
													if($reg->situacao != C_TicketSituacaoResolvido) echo $this->Html->link('<i class="fas fa-check"></i>', ["action" => "alterarsituacao", $reg->id, '2'], ['rel' => 'tooltip', 'title' => 'Resolvido', 'class' => 'btn btn-pgm btn-pgm-salvar btn-success btn-simple btn-xs', 'id' => $reg->id, 'escape' => false]);
												...
												echo $this->Html->link('<i class="fa fa-print"></i>', ["action" => "imprimir", $reg->id, "?" => ["autoprint" => 1]], ['rel' => 'tooltip', 'title' => 'Imprimir', 'target' => '_blank', 'class' => 'btn btn-pgm btn-pgm-imprimir btn-orange btn-simple btn-xs', 'id' => $reg->id, 'escape' => false]);
											?>
										</td>
									</tr>
								<?php endforeach; ?>
								...
								<?php foreach ($ticketsPendentes as $reg): ?>
									<tr rel="popover" data-trigger="hover" data-content='<div class="popover-big"><h4><?= AssuntoTicket($reg->assunto) ?> </h4><br><?= h($reg->solicitacao) ?></div>' data-original-title="Ticket <?= $reg->id.' ' ?><small class='pgm-popover-title-date'><i>(<?= date_format($reg->created, 'd/m/Y') ?>)</i></small>" data-html="true" data-placement="top">
										...
										<td class="td-actions">
											<?php
												if($reg->situacao != C_TicketSituacaoEmandamento) echo $this->Html->link('<i class="fas fa-reply"></i>', ["action" => "alterarsituacao", $reg->id, '1'], ['rel' => 'tooltip', 'title' => 'Em execução', 'class' => 'btn btn-pgm btn-pgm-situacao btn-info btn-simple btn-xs', 'id' => $reg->id, 'escape' => false]);
												echo $this->Html->link('<i class="fas fa-check"></i>', ["action" => "alterarsituacao", $reg->id, '2'], ['rel' => 'tooltip', 'title' => 'Resolvido', 'class' => 'btn btn-pgm btn-pgm-salvar btn-success btn-simple btn-xs', 'id' => $reg->id, 'escape' => false]);
							...
							<tbody>
								<?php foreach ($ticketsResolvidos as $reg): ?>
									<tr rel="popover" data-trigger="hover" data-content='<div class="popover-big"><h4><?= AssuntoTicket($reg->assunto) ?> </h4><br><?= h($reg->solicitacao) ?></div>' data-original-title="Ticket <?= $reg->id.' ' ?><small class='pgm-popover-title-date'><i>(<?= date_format($reg->created, 'd/m/Y') ?>)</i></small>" data-html="true" data-placement="top">
										...
										<td> <a class="link" target='_blank' href='<?= $this->Url->build(["controller" => "Tickets", "action" => "edit", $reg->id]) ?>'> <?php if($reg->cliente->tipo == 1){ echo $reg->cliente->nome;} else {echo $reg->cliente->razaosocial;} ?></a></td>
										<td class="td-actions">
											<?= $this->Html->link('<i class="fa fa-print"></i>', ["action" => "imprimir", $reg->id, "?" => ["autoprint" => 1]], ['rel' => 'tooltip', 'title' => 'Imprimir', 'target' => '_blank', 'class' => 'btn btn-pgm btn-pgm-imprimir btn-orange btn-simple btn-xs', 'id' => $reg->id, 'escape' => false]); ?>
										</td>
									</tr>
								<?php endforeach; ?>
							</tbody>
