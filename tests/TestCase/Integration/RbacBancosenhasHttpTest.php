...
/**
 * Stack HTTP + Rbac sobre Bancosenhas::index.
 * Requer pdo_sqlite e tests/bootstrap_http.php (suite rbac-http).
 */
class RbacBancosenhasHttpTest extends TestCase {
	...
	public function testGuestBancosenhasIndexRedirectsToLogin(): void {
		$this->get($this->bancosenhasIndexPath());
		$this->assertResponseCode(302);
		$this->assertRedirectContains('users/login');
	}

	public function testEquipeWithBancosenhasViewLoadsIndex(): void {
		self::rbacHttpSqliteSeedUser(self::$conn, 108, [
			'admin' => 0,
			'role' => 0,
			'idempresa' => 1,
		]);
		...
		self::$conn->execute(
			...
			['bancosenhas.view', 'Banco senhas ver', 'Banco de senhas', 'Bancosenhas', 'index', 0]
		);
		$pid = (int)self::$conn->lastInsertId();
		self::$conn->execute(
			'INSERT INTO rbac_roles (slug, name, is_system, active, sort_order, hierarchy_level) VALUES (?,?,?,?,?,?)',
			['http_bancosenhas_view', 'HTTP bancosenhas view', 0, 1, 1, 100]
		);
		...
		$this->session([
			'Auth' => [
				'User' => [
					'id' => 108,
					'username' => 'equipe_bancosenhas_http',
					'name' => 'Equipe banco senhas',
					'role' => 0,
					'admin' => 0,
					'idcliente' => null,
					'idempresa' => 1,
		...
		$this->assertResponseContains('Cofre de senhas');
		$this->assertResponseContains('Criptografado');
		$this->assertResponseContains('Nova credencial');
	}

	public function testEquipeWithOnlyMatrixPermissionDeniedOnBancosenhas(): void {
		self::rbacHttpSqliteSeedUser(self::$conn, 109, [
			'admin' => 0,
			'role' => 0,
			'idempresa' => 1,
		]);
		...
		self::$conn->execute(
			'INSERT INTO rbac_roles (slug, name, is_system, active, sort_order, hierarchy_level) VALUES (?,?,?,?,?,?)',
			['http_bancosenhas_denied', 'HTTP sem bancosenhas', 0, 1, 1, 100]
		);
		$rid = (int)self::$conn->lastInsertId();
		self::$conn->execute(
			'INSERT INTO rbac_roles_permissions (role_id, permission_id) VALUES (?,?)',
		...
		$this->session([
			'Auth' => [
				'User' => [
					'id' => 109,
					'username' => 'equipe_sem_bancosenhas_http',
					'name' => 'Equipe sem banco senhas',
					'role' => 0,
					'admin' => 0,
					'idcliente' => null,
					'idempresa' => 1,
	...
	protected function bancosenhasIndexPath(): string {
