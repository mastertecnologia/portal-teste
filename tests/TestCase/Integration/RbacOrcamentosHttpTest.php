...
/**
 * Stack HTTP + Rbac sobre Orcamentos::index (equipe).
 * Requer pdo_sqlite e tests/bootstrap_http.php (suite rbac-http).
 */
class RbacOrcamentosHttpTest extends TestCase {
	...
	public function testGuestOrcamentosIndexRedirectsToLogin(): void {
		$this->get($this->orcamentosIndexPath());
		$this->assertResponseCode(302);
		$this->assertRedirectContains('users/login');
	}

	public function testEquipeWithOrcamentosViewLoadsIndex(): void {
		self::rbacHttpSqliteSeedUser(self::$conn, 112, [
			'admin' => 0,
			'role' => 0,
			'idempresa' => 1,
		]);
		...
		$this->session([
			'Auth' => [
				'User' => [
					'id' => 112,
					'username' => 'equipe_orcamentos_http',
					'name' => 'Equipe orçamentos',
					'role' => 0,
					'admin' => 0,
					'idcliente' => null,
					'idempresa' => 1,
		...
		$this->assertResponseContains('Orçamentos');
		$this->assertResponseContains('Gerar Orçamento');
		$this->assertResponseContains('Pendentes');
	}

	public function testEquipeWithOnlyMatrixPermissionDeniedOnOrcamentos(): void {
		self::rbacHttpSqliteSeedUser(self::$conn, 113, [
			'admin' => 0,
			'role' => 0,
			'idempresa' => 1,
		]);
		...
		self::$conn->execute(
			'INSERT INTO rbac_roles (slug, name, is_system, active, sort_order, hierarchy_level) VALUES (?,?,?,?,?,?)',
			['http_orcamentos_denied', 'HTTP sem orcamentos', 0, 1, 1, 100]
		);
		$rid = (int)self::$conn->lastInsertId();
		self::$conn->execute(
			'INSERT INTO rbac_roles_permissions (role_id, permission_id) VALUES (?,?)',
		...
		$this->session([
			'Auth' => [
				'User' => [
					'id' => 113,
					'username' => 'equipe_sem_orcamentos_http',
					'name' => 'Equipe sem orçamentos',
					'role' => 0,
					'admin' => 0,
					'idcliente' => null,
					'idempresa' => 1,
	...
	protected function orcamentosIndexPath(): string {
