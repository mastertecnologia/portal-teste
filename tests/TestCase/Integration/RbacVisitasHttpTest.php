...
/**
 * Stack HTTP + Rbac sobre Visitas::index (equipe).
 * Requer pdo_sqlite e tests/bootstrap_http.php (suite rbac-http).
 */
class RbacVisitasHttpTest extends TestCase {
	...
	public function testGuestVisitasIndexRedirectsToLogin(): void {
		$this->get($this->visitasIndexPath());
		$this->assertResponseCode(302);
		$this->assertRedirectContains('users/login');
	}

	public function testEquipeWithVisitasViewLoadsIndex(): void {
		self::rbacHttpSqliteSeedUser(self::$conn, 116, [
			'admin' => 0,
			'role' => 0,
			'idempresa' => 1,
		]);
		...
		$this->assertResponseContains('Cadastrar nova visita');
		$this->assertResponseContains('Calendário');
		$this->assertResponseContains('Situação');
	}

	public function testEquipeWithOnlyMatrixPermissionDeniedOnVisitas(): void {
		self::rbacHttpSqliteSeedUser(self::$conn, 117, [
			'admin' => 0,
			'role' => 0,
			'idempresa' => 1,
		]);
		...
		self::$conn->execute(
			'INSERT INTO rbac_roles (slug, name, is_system, active, sort_order, hierarchy_level) VALUES (?,?,?,?,?,?)',
			['http_visitas_denied', 'HTTP sem visitas', 0, 1, 1, 100]
		);
		$rid = (int)self::$conn->lastInsertId();
		self::$conn->execute(
			'INSERT INTO rbac_roles_permissions (role_id, permission_id) VALUES (?,?)',
		...
		$this->session([
			'Auth' => [
				'User' => [
					'id' => 117,
					'username' => 'equipe_sem_visitas_http',
					'name' => 'Equipe sem visitas',
					'role' => 0,
					'admin' => 0,
					'idcliente' => null,
					'idempresa' => 1,
	...
	protected function visitasIndexPath(): string {
